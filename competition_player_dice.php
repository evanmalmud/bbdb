...
<?php
	...
	function do_start_of_row($totals_row) {
		echo "<tr>";
		table_cell_with_link($totals_row['player_name'], "player.php?player_id=" . $totals_row['player_id']);
		table_cell_with_link($totals_row['team_name'], "team.php?team_id=" . $totals_row['team_id']);
		table_cell_with_link($totals_row['player_type'], "player_type.php?player_type_id=" . $totals_row['player_type_id']);
	...
	$totals_sql = $conn->prepare("
			SELECT p.player_id, p.description AS player_name
				, t.team_id, t.description AS team_name
				, rt.roll_type_id, rt.description AS roll_type
				, pt.player_type_id, pt.description AS player_type
			...
			GROUP BY p.player_id, t.team_id, rt.roll_type_id, pt.player_type_id
			ORDER BY rt.description, t.description, p.description, p.player_id
	");
	...
	$main_sql = $conn->prepare("
			...
			WHERE mc.competition_id = ?
			AND rt.dice_type_id = 1
			GROUP BY p.player_id, t.team_id, rt.roll_type_id, pt.player_type_id, ml.roll_value
			ORDER BY rt.description, t.description, p.description, p.player_id, ml.roll_value
	...
	echo '<thead><tr><th>Player</th><th>Team</th><th>Position</th><th>Roll Type</th><th>1</th><th>2</th><th>3</th><th>4</th><th>5</th><th>6</th><th>Total</th><th>Successes</th></thead>';
	echo '<tbody>';
	...
	$last_player_id = $totals_row['player_id']; // this means the first loop will work

	while ($row = $main_sql->fetch(PDO::FETCH_ASSOC)) {
			
		if($last_player_id<>$row['player_id']) { // it's a new roll type.... so do a new row, after padding out to & doing the total # of rolls??
